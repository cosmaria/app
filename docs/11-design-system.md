...
| `color.text.on-critical` | `#FFFFFF` | `#FFFFFF` |
| `color.border` | `#232D38` | `#DDE3E8` |

> `color.text.on-accent` representa texto/ícone exibido **sobre uma superfície Accent** (ex.: botão primário). É branco nos dois temas porque o accent é saturado tanto em Dark quanto em Light. No código, a chave equivalente é `color.text.onAccent` (camelCase, mesma convenção de `bg.surface-2` ↔ `bg.surface2`).
>
> `color.text.on-critical` (código: `color.text.onCritical`) representa texto/ícone **sobre uma superfície semântica crítica** (ex.: botão destrutivo, sobre `semantic.critical`). É uma **função semântica distinta** de `on-accent` — o destrutivo fica sobre `critical`, nunca sobre um accent. Ambos são brancos hoje, mas **permanecem independentes**: `on-critical` nunca é alias de `on-accent`, e cada um pode evoluir por conta própria sem afetar o outro.
